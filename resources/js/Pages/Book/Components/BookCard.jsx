...
export default function BookCard({ book }) {
    ...
    const handleRemoveBook = (book_id) => {
        router.post(route('search.books.destroy'), { book_id }, {
            preserveScroll: true,
            onBefore: () => {
                setLoading(true)
            },
            ...
            <div className="w-full flex flex-col justify-between p-5 h-full">
                ...
                <div className="flex item-center justify-between mt-6 ">
                    ...
                    <div className="flex items-center gap-2">
                        {user_cart?.cart?.length > 0 && user_cart.cart.some(b => b.book_id == book.book.id) ?
                            <DangerButton disabled={loading} onClick={() => handleRemoveBook(book.book.id)}>
